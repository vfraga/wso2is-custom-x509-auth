...
@PrepareForTest({
  X509CertificateAuthenticator.class,
...
public class X509CertificateAuthenticatorTest {
  ...
  private static final String CERT_WITH_ALT_NAMES =
  ...
  private static final String CERT_WITH_SIMILAR_ALT_NAMES =
  ...
  private static final String CERT_WITH_ONE_CN_NO_ALT_NAMES =
  ...
  private static final String CERT_WITH_COMMA_SEPARATED_CN =
  ...
  public Object[][] provideTestData() throws Exception {
        ...
        (X509Certificate)
            factory.generateCertificate(
                new ByteArrayInputStream(
                    DatatypeConverter.parseBase64Binary(CERT_WITH_NO_ALT_NAMES)));
    ...
    parameterMap1.put(
        X509CertificateConstants.ALTERNATIVE_NAMES_REGEX_CONFIG_PROPERTY,
        "^[a-zA-Z]{3}.[a-zA-Z]{2}$");
    parameterMap1.put(
        X509CertificateConstants.USER_NAME_REGEX_CONFIG_PROPERTY, "^[a-zA-Z]{3}.[a-zA-Z]{2}$");
    parameterMap1.put(X509CertificateConstants.USERNAME, "CN");
    parameterMap1.put(
        X509CertificateConstants.AUTHENTICATION_ENDPOINT_CONFIG_PROPERTY,
    ...
    X509Certificate cert2 =
        (X509Certificate)
            factory.generateCertificate(
                new ByteArrayInputStream(DatatypeConverter.parseBase64Binary(CERT_WITH_ALT_NAMES)));
    ...
    authenticatorConfig2.setParameterMap(parameterMap2);

    // Authenticate using the username attribute when no pattern configurations
    X509Certificate cert3 =
        (X509Certificate)
            factory.generateCertificate(
                new ByteArrayInputStream(
                    DatatypeConverter.parseBase64Binary(CERT_WITH_ONE_CN_NO_ALT_NAMES)));
    ...
    X509Certificate cert4 =
        (X509Certificate)
            factory.generateCertificate(
                new ByteArrayInputStream(
                    DatatypeConverter.parseBase64Binary(CERT_WITH_COMMA_SEPARATED_CN)));
    ...
    // Authenticate using the username attribute when the pattern is configured
    ...
    parameterMap5.put(
        X509CertificateConstants.USER_NAME_REGEX_CONFIG_PROPERTY, "\\d\\d\\d[a-zA-Z]{8}");
    authenticatorConfig5.setParameterMap(parameterMap5);

    // Authenticate using the email attribute when the pattern is configured
    X509Certificate cert6 =
        (X509Certificate)
            factory.generateCertificate(
                new ByteArrayInputStream(
                    DatatypeConverter.parseBase64Binary(CERT_WITH_SUBJECT_IN_THE_EMAIL)));
    ...
    parameterMap6.put(X509CertificateConstants.USER_NAME_REGEX_CONFIG_PROPERTY, "[a-zA-Z]{3}\\d");
    authenticatorConfig6.setParameterMap(parameterMap6);

    // Authenticate Using similar alternative names.
    X509Certificate cert7 =
        (X509Certificate)
            factory.generateCertificate(
                new ByteArrayInputStream(
                    DatatypeConverter.parseBase64Binary(CERT_WITH_SIMILAR_ALT_NAMES)));
    ...
    parameterMap7.put(X509CertificateConstants.ALTERNATIVE_NAMES_REGEX_CONFIG_PROPERTY, "^.*wso2$");
    parameterMap7.put(
        X509CertificateConstants.USER_NAME_REGEX_CONFIG_PROPERTY, "^[a-zA-Z]{3}.[a-zA-Z]{2}$");
    ...
    return new Object[][] {
      ...
      {
        certificateArrayObject4,
        authenticatorConfig4,
        sequenceConfig4,
        true,
    ...
    if (exceptionShouldThrown) {
      try {
        ...
        if (!exception.getMessage().equals(exceptionMessage)) {
          fail(
              "expected exception message was: "
                  + exceptionMessage
                  + " but actual was: "
  ...
  @Test
  public void testProcessAuthenticationResponseWithAccountLock() throws Exception {
    HttpServletRequest mockRequest = mock(HttpServletRequest.class);
    AuthenticationContext authenticationContext = new AuthenticationContext();
    HttpServletResponse mockResponse = mock(HttpServletResponse.class);
    ...
    X500Principal principal = new X500Principal("CN=testuser");
    ...
    when(X509CertificateUtil.getUserStoreDomainName(anyString(), anyString(), any()))
        .thenReturn("PRIMARY");
    when(X509CertificateUtil.isAccountLocked(any(AuthenticatedUser.class))).thenReturn(true);
    ...
    try {
      spy.process(mockRequest, mockResponse, authenticationContext);
      fail("Expected AuthenticationFailedException due to account lock");
  ...
  @Test
  public void testProcessAuthenticationResponseWithAccountDisabled() throws Exception {
    HttpServletRequest mockRequest = mock(HttpServletRequest.class);
    AuthenticationContext authenticationContext = new AuthenticationContext();
    HttpServletResponse mockResponse = mock(HttpServletResponse.class);
    ...
    when(X509CertificateUtil.getUserStoreDomainName(anyString(), anyString(), any()))
        .thenReturn("PRIMARY");
    when(X509CertificateUtil.isAccountDisabled(any())).thenReturn(true);

    try {
      spy.process(mockRequest, mockResponse, authenticationContext);
      fail("Expected AuthenticationFailedException due to account disabled");
    } catch (AuthenticationFailedException e) {
      assertEquals(
          authenticationContext.getProperty(
              X509CertificateConstants.X509_CERTIFICATE_ERROR_CODE_CONTEXT_PROPERTY),
